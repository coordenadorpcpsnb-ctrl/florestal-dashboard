...
const CONTEXTO = join(__dirname, "contexto-mercado.json");
...
// ---- Bloco CONTEXTO (analise de mercado, de contexto-mercado.json) ----
if (existsSync(CONTEXTO)) {
  try {
    const ctx = JSON.parse(readFileSync(CONTEXTO, "utf-8"));
    const esc = (t) => String(t).replace(/`/g, "\\`").replace(/\$\{/g, "\\${");
    // Leitura automatica do ultimo fechamento — derivada dos NOSSOS numeros (nao inventa nada)
    const sinal = (x) => x == null ? null : (x > 0 ? "alta" : x < 0 ? "queda" : "estabilidade");
    const fmt = (x) => x == null ? null : `${x > 0 ? "+" : ""}${x.toFixed(1).replace(".", ",")}%`;
    const partes = [];
    for (const [k, nome] of [["ureia","ureia"],["map","MAP"],["kcl","KCl"]]) {
      if (delta[k] != null) partes.push(`${nome} ${fmt(delta[k])}`);
    }
    const leituraAuto = partes.length
      ? `Último fechamento (${refCurta(refs.ureia ?? refs.map ?? refs.kcl)}): ${partes.join(", ")} ante o mês anterior.`
      : `Sem novo fechamento de fertilizantes desde a leitura anterior (${refCurta(refs.ureia ?? refs.map ?? refs.kcl)}).`;

    // Noticias recentes (reais, com link) vindas do data.json
    const noticias = (d.noticiasFert ?? []).slice(0, 4).map(n => ({
      data: n.data ?? "", titulo: n.titulo ?? "", url: n.url ?? "",
    }));

    const obj = {
      atualizadoEm: ctx.atualizadoEm ?? "",
      titulo: ctx.titulo ?? "Contexto de mercado",
      resumo: ctx.resumo ?? "",
      causas: (ctx.causas ?? []).map(x => ({ titulo: x.titulo ?? "", texto: x.texto ?? "" })),
      perspectiva: ctx.perspectiva ?? [],
      nota: ctx.nota ?? "",
      leituraAuto,
      noticias,
    };
    const js = "const CONTEXTO = " + JSON.stringify(obj, null, 2).replace(/\$\{/g, "\\${") + ";";
    const reCtx = /\/\* CONTEXTO-START \*\/[\s\S]*?\/\* CONTEXTO-END \*\//;
    if (reCtx.test(html)) {
      html = html.replace(reCtx, `/* CONTEXTO-START */\n${js}\n/* CONTEXTO-END */`);
      console.log(`[patch] bloco CONTEXTO atualizado (rev. ${obj.atualizadoEm}).`);
    }
  } catch (e) { console.log("[patch] contexto-mercado.json invalido, mantendo o anterior:", e.message); }
}
