...
const CTX = existsSync(join(__dirname, "contexto-mercado.json"))
  ? JSON.parse(readFileSync(join(__dirname, "contexto-mercado.json"), "utf-8")) : null;
...
const pct=(x)=>x==null?"—":(x>0?"+":"")+x.toFixed(1).replace(".",",")+"%";
const colorOf=(x)=>x==null||Math.abs(x)<0.3?MUTE:(x>0?BORD:EMER); // custo: alta=vermelho, queda=verde

// "2026-06" -> "jun/26"
const MES_ABREV=["jan","fev","mar","abr","mai","jun","jul","ago","set","out","nov","dez"];
const refCurta=(r)=>{const m=/^(\d{4})-(\d{2})$/.exec(r??"");return m?`${MES_ABREV[Number(m[2])-1]}/${m[1].slice(2)}`:(r??"—");};
const refs=d.refsFertilizantes??{};
// Fertilizantes fecham 1x por mes: sem novo fechamento, a coluna explica em vez de mostrar 0,0%
const obsFert=(k,padrao)=>delta[k]==null?`Fechamento ${refCurta(refs[k])} (sem novo dado)`:`${padrao} · ref. ${refCurta(refs[k])}`;
...
const fertTable=new Table({width:{size:W,type:WidthType.DXA},columnWidths:[2600,1900,1900,2960],rows:[
  ...
  new TableRow({children:[td("Ureia",2600),td(`US$ ${int(c.ureia)}/t`,1900,AlignmentType.RIGHT),td([v(pct(delta.ureia),colorOf(delta.ureia))],1900,AlignmentType.RIGHT),td([new TextRun({text:obsFert("ureia","Nitrogenado"),size:17})],2960)]}),
  new TableRow({children:[td("MAP",2600),td(`US$ ${int(c.map)}/t`,1900,AlignmentType.RIGHT),td([v(pct(delta.map),colorOf(delta.map))],1900,AlignmentType.RIGHT),td([new TextRun({text:obsFert("map","Fosfatado"),size:17})],2960)]}),
  new TableRow({children:[td("Cloreto de Potássio (KCl)",2600),td(`US$ ${int(c.kcl)}/t`,1900,AlignmentType.RIGHT),td([v(pct(delta.kcl),colorOf(delta.kcl))],1900,AlignmentType.RIGHT),td([new TextRun({text:obsFert("kcl","Potássico"),size:17})],2960)]}),
]});
  ...
  sections:[{
    ...
    children:[
      ...
      new Paragraph({spacing:{after:120},alignment:AlignmentType.JUSTIFIED,children:[
        ...
        new TextRun({text:`Ureia FOB em US$ ${int(c.ureia)}/t${delta.ureia==null?` (fechamento ${refCurta(refs.ureia)})`:` (${pct(delta.ureia)})`}, MAP US$ ${int(c.map)}/t e KCl US$ ${int(c.kcl)}/t. `,size:19}),
      ...
      ...(CTX ? [
        h2("6. Contexto de Mercado — Causas da Alta e Perspectivas"),
        new Paragraph({spacing:{after:60},children:[new TextRun({text:`${CTX.titulo} · atualizado em ${CTX.atualizadoEm} · revisão manual.`,bold:true,size:18,color:NAVY})]}),
        ...(CTX.resumo ? [new Paragraph({spacing:{after:60},alignment:AlignmentType.JUSTIFIED,children:[new TextRun({text:CTX.resumo,size:18})]})] : []),
        // Leitura automatica derivada dos nossos numeros (nao inventa nada)
        ...(() => {
          const fmt=(x)=>x==null?null:`${x>0?"+":""}${x.toFixed(1).replace(".",",")}%`;
          const ps=[]; for(const [k,nome] of [["ureia","Ureia"],["map","MAP"],["kcl","KCl"]]) if(delta[k]!=null) ps.push(`${nome} ${fmt(delta[k])}`);
          const txt = ps.length ? `Leitura automática — último fechamento (${refCurta(refs.ureia??refs.map??refs.kcl)}): ${ps.join(", ")} ante o mês anterior.`
                                : `Leitura automática: sem novo fechamento de fertilizantes desde a leitura anterior (${refCurta(refs.ureia??refs.map??refs.kcl)}).`;
          return [new Paragraph({spacing:{after:80},children:[new TextRun({text:txt,size:17,italics:true,color:MUTE})]})];
        })(),
        new Paragraph({spacing:{after:40},children:[new TextRun({text:"Principais causas da alta:",bold:true,size:18,color:NAVY})]}),
        ...(CTX.causas||[]).map(x=>bullet([new TextRun({text:`${x.titulo}: `,bold:true,size:17}),new TextRun({text:x.texto,size:17})])),
        ...(CTX.numeros ? [new Paragraph({spacing:{before:40,after:80},alignment:AlignmentType.JUSTIFIED,children:[new TextRun({text:CTX.numeros,size:17})]})] : []),
        new Paragraph({spacing:{before:40,after:40},children:[new TextRun({text:"O que esperar nos próximos meses:",bold:true,size:18,color:NAVY})]}),
        ...(CTX.perspectiva||[]).map(x=>bullet([new TextRun({text:x,size:17})])),
        ...((d.noticiasFert?.length) ? [
          new Paragraph({spacing:{before:60,after:20},children:[new TextRun({text:"Notícias recentes do mercado (atualização automática):",bold:true,size:17,color:NAVY})]}),
          ...d.noticiasFert.slice(0,4).map(n=>bullet([new TextRun({text:`${n.data?`${n.data} — `:""}${n.titulo}`,size:16})])),
        ] : []),
        ...(CTX.fontes?.length ? [new Paragraph({spacing:{before:60,after:20},children:[new TextRun({text:"Fontes desta análise:",bold:true,size:16,color:MUTE})]}),
          new Paragraph({spacing:{after:40},children:[new TextRun({text:CTX.fontes.map(f=>`${f.nome} (${f.data})`).join("; ")+".",size:15,color:MUTE})]})] : []),
        ...(CTX.nota ? [new Paragraph({spacing:{after:40},children:[new TextRun({text:CTX.nota,italics:true,size:15,color:MUTE})]})] : []),
      ] : []),
